...
> · TGS Ref No: **TGS-2022017524** · 3-day course. The Learner Guide is generated by
> `.claude/skills/wsq-learner-guide/build_learner_guide.py` → `courseware/LG-<course>.docx` + `.pdf`, and — as a
> project-specific exception to HARD RULE 1 — a Markdown mirror `LEARNER-GUIDE.md` at the repo
> root is REQUIRED here (it is published to GitHub for learners) and must stay 100% aligned
> with the DOCX and the labs in `labs/`.
