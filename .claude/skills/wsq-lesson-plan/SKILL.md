...
> · TGS Ref No: **TGS-2022017524** · 3-day course. The Lesson Plan is generated by
> `.claude/skills/wsq-lesson-plan/build_lesson_plan.py` → `courseware/LP-<course>.docx` + `.pdf`. Its **Slides**
> column must match `courseware/facilitator-slides.pptx` page numbers and the labs in `labs/`.
> Day 3 ends with the 2-hour assessment block (1 hr WA + 1 hr PP) finishing at 6:00 PM.
