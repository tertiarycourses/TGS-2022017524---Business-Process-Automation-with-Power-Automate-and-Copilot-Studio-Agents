...
> · TGS Ref No: **TGS-2022017524** · 2-day course. The Lesson Plan is generated by
> `.claude/skills/wsq-lesson-plan/build_lesson_plan.py` → `courseware/LP-<course>.docx` + `.pdf`. Its **Slides**
> column must match `courseware/Business Process Automation with Power Automate and Copilot Studio Agents-v3.pptx` page numbers and the labs in `labs/`.
> Day 2 ends with the 2-hour assessment block (1 hr WA + 1 hr PP) finishing at 6:00 PM.
