...
1. Select **+** → **Add an action** → **Move or rename a file** (OneDrive for Business):
   - **File:** insert **Dynamic content → File identifier** (from the trigger)
   - **Destination File Path:** use the picker to select `Invoices/Approved`, then type `/` and insert **Dynamic content → File name** — the path must end with the file name
   - **Overwrite:** `Yes`
...
1. Select **+** → **Add an action** → **Move or rename a file**:
   - **File:** **Dynamic content → File identifier**
   - **Destination File Path:** pick `Invoices/Rejected`, then `/` + **File name** (as above)
   - **Overwrite:** `Yes`
...
3. The flow triggers and pauses at the approval. Respond in **one** of these ways:
   - Open the **Approvals** hub from the left menu of make.powerautomate.com (look under **More** if it is not pinned), open the request, and choose **Approve**.
