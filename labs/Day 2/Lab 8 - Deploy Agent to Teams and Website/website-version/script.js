/* ============================================================================
   ...
   Lab 8 — Power Automate HTTP flow version

   The Power Automate HTTP POST URL is entered in the lab configuration panel,
   so you never have to edit this file. It is saved to localStorage.

   Save and turn on the flow before using its URL. The Response action must
   return JSON and include an Access-Control-Allow-Origin header.

   DEFAULT_WEBHOOK_URL just pre-fills the box; the panel always wins.
   ========================================================================== */
const DEFAULT_WEBHOOK_URL = "";
const STORAGE_KEY = "mtb.powerAutomateUrl";

/* Copy the decisions the Power Automate flow can return for card styling. */
const DECISIONS = {
  APPROVED: {
    cls: "is-approved",
    icon: "✓",
...
function renderWebhookStatus() {
  ...
  if (!url) {
    ...
    webhookStatus.textContent = "HTTPS endpoint detected. Save and turn on the Power Automate flow before submitting.";
    webhookStatus.className = "config-status is-ok";
  } else {
    webhookStatus.textContent = "Enter the complete HTTPS URL generated by the Power Automate trigger.";
    webhookStatus.className = "config-status is-warn";
  }
}
...
document.getElementById("demoCase").addEventListener("change", (e) => {
  ...
  // Never autofill an email — the flow sends a real message to whatever is here.
...
form.addEventListener("submit", async (event) => {
  ...
  if (!endpoint) {
    ...
    errorBox.textContent = "Enter your Power Automate HTTP POST URL in the Lab configuration panel first.";
    errorBox.hidden = false;
  ...
  try {
    const response = await fetch(endpoint, {
      method: "POST",
      // text/plain keeps this a simple cross-origin request. The flow converts
      // the body to JSON with json(triggerBody()).
    ...
    // Power Automate's Response action returns the flow's structured decision.
    ...
    errorBox.textContent =
      ...
      `(Trainer: check the HTTP POST URL, that the flow is on, and that the Response action includes the CORS header.)`;
    errorBox.hidden = false;
...
function showResult(decision) {
  ...
  const style = DECISIONS[key] || {
    ...
    fallback: "We could not read the decision. Check the Power Automate run history."
  };
  ...
  flagList.replaceChildren(...flagItems);
